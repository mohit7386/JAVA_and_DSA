...
class Task{
    ...
    //Applying toString() method when we want to print the object then we need to override toString method for redeable output
    @Override
...
public class Linkedlist5 {
    public static void main(String[] args){
        ...
        //creating a infinite loop jisse jab tak user exit nahi karega tab tak ye run hota rahega 
        while(true){
            ...
            switch (choice) {
                case 1:
                    ...
                    taskList.addLast(t1); // Normal task at the end
                    System.out.println("[Normal Task added at end]");
                    break;
                case 2:
                    Task t2 = createTask(sc, "High");
                    taskList.addFirst(t2); // High priority task at the start
                    System.out.println("[High Priority Task added at start]");
                    break;
                case 3:
                    if (!taskList.isEmpty()) {
                        Task resolved = taskList.removeFirst(); // Resolving task (removing from front)
