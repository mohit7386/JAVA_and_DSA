...
Ab Step 1: Pehle Set Interface ka concept samjhte hain:
🔥 What is Set in Java Collection Framework?

Feature	Details
Definition	Set is a collection that does not allow duplicate elements.
Key Point	- No duplicates allowed.
- Order is not guaranteed (except LinkedHashSet & TreeSet).
Interfaces	Set is a child interface of Collection.
Real World Example	- Set of unique student roll numbers.
- Set of unique mobile numbers.
- Set of different countries.
...
Type → Integer, String, Double, etc. (jo data store karna hai)

🔥 Properties of Set:
No duplicate elements allowed.

At most one null element is allowed (HashSet, LinkedHashSet allow, TreeSet does not allow null).

No guarantee of insertion order (HashSet).

Maintains insertion order (LinkedHashSet).

Sorted in natural order (TreeSet).

🔥 Set vs List (Quick Comparison):

List	                                           Set
Duplicates allowed.	             ->         Duplicates not allowed.
Elements are indexed.	         ->         No indexing.
Maintains insertion order (ArrayList). ->	Order not guaranteed (except LinkedHashSet, TreeSet).

1. What is HashSet?

Point	                   Explanation
Definition	->        HashSet is a class in Java that implements the Set Interface and stores unique elements.
Duplicate Elements -> ❌ Not allowed (it removes duplicates automatically).
Insertion Order	  ->  ❌ Not maintained (elements ka koi fix order nahi hota).
Null Elements ->      ✅ Only one null element allowed.
Internal Working ->	  ✅ Uses HashMap internally to store elements. (value = dummy constant)
...
<> → Generics: jo type ka data rakhna hai wo mention karte hain.

Import karna padta hai java.util.HashSet package.
...
[Banana, Apple, Orange]
(Order random ho sakta hai because no insertion order maintained)

4. Important Methods of HashSet

Method	Description
add(element)	Adds the element if not already present.
...
5. Internal Working of HashSet (Deep Industry Level):
Jab hum set.add(element) karte hain:

Element ka hashCode calculate hota hai.

HashCode se uska bucket decide hota hai (hashing technique).

Agar bucket me already wahi element nahi hai, to add kar diya jaata hai.

Agar duplicate detect hota hai (hashCode + equals dono match), to add nahi karta.

NOTE:

Isiliye hashCode() aur equals() ka concept important hota hai Set me.

HashSet internally ek hidden HashMap ka use karta hai:

private transient HashMap<E,Object> map;
jisme element key ban jaata hai aur value ek dummy constant hoti hai (PRESENT).

6. Real World Use Case of HashSet
Unique Student IDs
